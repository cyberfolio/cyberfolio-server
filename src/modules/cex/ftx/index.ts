...
const getAssets = async ({ apiKey, apiSecret }: { apiKey: string; apiSecret: string }): Promise<CexAssetResponse[]> => {
  ...
  try {
    ...
    const assets = allBalances?.data?.result?.main?.filter((balance) => Number(balance.usdValue) > 1);
    const response = [];
    if (Array.isArray(assets) && assets.length > 0) {
      for (const asset of assets) {
        const balance = roundNumber(asset.total);
        const symbol = asset.coin?.toLowerCase();
