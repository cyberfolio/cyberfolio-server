...
// import {
//  Connection, clusterApiUrl, PublicKey, Cluster
// } from '@solana/web3.js';
...
//             solanaResponse.data[data]?.result?.value[0]?.account?.data?.parsed?.info?.tokenAmount?.amount,
//           solanaDecimals;
//           response.push({
...
//         const balance = Number(
//           solanaResponse.data[i]?.result?.value[0]?.account?.data?.parsed?.info?.tokenAmount?.amount,
//         solanaDecimals;
//         response.push({
